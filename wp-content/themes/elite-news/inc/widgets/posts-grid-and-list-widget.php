<?php
if ( ! class_exists( 'Elite_News_Posts_Grid_And_List_Widget' ) ) {
	/**
	 * Adds Elite_News_Posts_Grid_And_List_Widget Widget.
	 */
	class Elite_News_Posts_Grid_And_List_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$elite_news_grid_list_widget_ops = array(
				'classname'   => 'ascendoor-widget magazine-grid-list-section style-1',
				'description' => __( 'Retrive Posts Grid And List Widgets', 'elite-news' ),
			);
			parent::__construct(
				'elite_news_magazine_grid_list_widget',
				__( 'Ascendoor Grid And List Widget', 'elite-news' ),
				$elite_news_grid_list_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$grid_list_post_count   = isset( $instance['number'] ) ? absint( $instance['number'] ) : 5;
			...
			<div class="section-header">
				...
				if ( ! empty( $grid_list_button_label ) ) {
					...
					<a href="<?php echo esc_url( $grid_list_button_link ); ?>" class="mag-view-all-link">
						<?php echo esc_html( $grid_list_button_label ); ?>
					</a>
					<?php
				}
				?>
			...
			<div class="magazine-section-body">
				<div class="magazine-grid-list-section-wrapper">
					...
					$magazine_grid_list_widgets_args = array(
						'post_type'      => 'post',
						'posts_per_page' => absint( $grid_list_post_count ),
					...
					if ( $query->have_posts() ) :
						...
						while ( $query->have_posts() ) :
							$query->the_post();
							?>
							<div class="mag-post-single has-image <?php echo esc_attr( 1 === $i ? '' : 'list-design' ); ?>">
		...
		public function form( $instance ) {
			...
			<p>
				...
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="6" value="<?php echo absint( $grid_list_post_count ); ?>" size="3" />
			</p>
			<p>
