	...
	class Elite_News_Categories_Widget extends WP_Widget {
		...
		public function __construct() {
			$elite_news_category_widget_ops = array(
				...
				'description' => __( 'Retrive Categories Widgets', 'elite-news' ),
			);
			parent::__construct(
				'elite_news_magazine_category_widget',
				__( 'Ascendoor Categories Widget', 'elite-news' ),
						...
						<div class="category-single">
							...
							<a href="<?php echo esc_url( get_term_link( $categories->term_id ) ); ?>">
								...
								<span class="category-no">
									...
									<span class="posts"><?php esc_html_e( 'Posts', 'elite-news' ); ?></span>
								</span>
							</a>
						</div>
						<?php
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of category to show(Min:1 | Max:4):', 'elite-news' ); ?></label>
			...
			for ( $i = 1; $i <= $category_count; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'category_' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Select category %d:', 'elite-news' ), $i ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'category_image_' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Category Image %s :', 'elite-news' ), $i ); ?></label><br/>
					...
					<input type="button" class="select-img button" value="<?php esc_attr_e( 'Upload', 'elite-news' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'elite-news' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'elite-news' ); ?>" style="margin-top:5px;" /><br/>
				</p>
 ...
 **/
function elite_news_pro_widget_image_upload_enqueue( $hook ) {
...
add_action( 'admin_enqueue_scripts', 'elite_news_pro_widget_image_upload_enqueue' );
