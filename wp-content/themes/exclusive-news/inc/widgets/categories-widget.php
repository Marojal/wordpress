<?php
if ( ! class_exists( 'Elite_News_Categories_Widget' ) ) {
	/**
	 * Adds Elite_News_Categories_Widget Widget.
	 */
	class Elite_News_Categories_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$elite_news_category_widget_ops = array(
				'classname'   => 'ascendoor-widget magazine-category-section',
				'description' => __( 'Retrive Categories Widgets', 'exclusive-news' ),
			);
			parent::__construct(
				'elite_news_magazine_category_widget',
				__( 'Ascendoor Categories Widget', 'exclusive-news' ),
				$elite_news_category_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			<?php } ?>
			<div class="magazine-child-category-wrapper">
				<?php
					...
					if ( ! empty( $categories->name ) ) {
						...
						<div class="category-single <?php echo esc_attr( empty( $category_image ) ? 'no-category-image' : '' ); ?>">
							...
							<a href="<?php echo esc_url( get_term_link( $categories->term_id ) ); ?>">
								<div class="category-title"><?php echo esc_html( $categories->name ); ?></div>
								<div class="category-count-wrap">
									<span class="category-icon">
										<i class="fa solid fa-arrow-right-long"></i>
									</span>
									<?php
									$post_count = $categories->count <= 1 ? $categories->count . __( ' Post', 'exclusive-news' ) : $post_count = $categories->count . __( ' Posts', 'exclusive-news' );
									?>
									<span class="category-no"><?php echo esc_html( $post_count ); ?></span>
								</div>
							</a>
						</div>
						<?php
		...
		public function form( $instance ) {
			...
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Section Title:', 'exclusive-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of category to show(Min:1 | Max:4):', 'exclusive-news' ); ?></label>
			...
			for ( $i = 1; $i <= $category_count; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'category_' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Select category %d:', 'exclusive-news' ), $i ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'category_image_' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Category Image %s :', 'exclusive-news' ), $i ); ?></label><br/>
					...
					<input type="button" class="select-img button" value="<?php esc_attr_e( 'Upload', 'exclusive-news' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'exclusive-news' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'exclusive-news' ); ?>" style="margin-top:5px;" /><br/>
				</p>
 ...
 *
 * @since Elite News 1.0
 **/
function elite_news_widget_image_upload_enqueue( $hook ) {
	...
	wp_enqueue_media();
	wp_enqueue_script( 'widget-image-upload-script', get_template_directory_uri() . '/assets/js/upload.min.js', array( 'jquery' ), ELITE_NEWS_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'elite_news_widget_image_upload_enqueue' );
